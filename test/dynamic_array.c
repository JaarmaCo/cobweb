...
static void run_test(void (*runner)(dynamic_array_i *da),
...
static void check_append(dynamic_array_i *da) {
  const int VALUES[] = {
      1, 2, 3, 4, 5, 6, 7,
  };
  ...
  for (size_t i = 0; i < sizeof VALUES / sizeof *VALUES; ++i) {
    int *value = da_append_i(da, VALUES[i]);
    if (NULL == value) {
...
static void check_append_range(dynamic_array_i *da) {
  ...
  for (size_t i = 0; i < sizeof VALUES / sizeof *VALUES; ++i) {
    ...
    int *first = da_append_range_i(da, SIZES[i], VALUES[i]);
    if (NULL == first) {
      fprintf(stderr, ERR("NULL returned by da_append_range_i"));
      exit(1);
    }
...
static void check_insert(dynamic_array_i *da) {
  printf(INFO("--- Test check_insert"));

  const int VALUES[] = {0, 1, 2, 4, 6, 7};
  da_append_range_i(da, 6, VALUES);

  int *item = da_insert_i(da, 3, 3);
  if (NULL == item) {
    fprintf(stderr, ERR("da_insert_i returned NULL"));
    exit(1);
  }
  if (*item != 3) {
    fprintf(stderr, ERR("da_insert_i return value did not correspond to the "
                        "inserted value."));
    exit(1);
  }
  item = da_insert_i(da, 5, 5);
  if (NULL == item) {
    fprintf(stderr, ERR("da_insert_i returned NULL"));
    exit(1);
  }
...
static void check_remove(dynamic_array_i *da) {
  printf(INFO("--- Test check_remove"));
  const int VALUES[] = {0,   1,   2,   3, 100, 200, 300, 400,
                        500, 600, 700, 4, 5,   6,   7};
  da_append_range_i(da, sizeof VALUES / sizeof *VALUES, VALUES);
  da_remove_range_i(da, 4, 7);
}
...
int main(void) {
  ...
  run_test(check_insert, (const int[]){0, 1, 2, 3, 4, 5, 6, 7}, 8);
